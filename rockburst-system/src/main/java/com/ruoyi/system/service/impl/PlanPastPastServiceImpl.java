...
@Service
@Transactional
public class PlanPastPastServiceImpl extends ServiceImpl<PlanPastMapper, PlanPastEntity> implements PlanPastService {
     ...
     */
    @Override
    public int insertPlan(PlanPastDTO planDTO) {
        int flag = 0;
        // 参数校验
        checkParameter(planDTO);
        if (ObjectUtil.isNotNull(planDTO.getPlanName())) {
            // 计划名称不能重复
            Long selectCount = planPastMapper.selectCount(new LambdaQueryWrapper<PlanPastEntity>()
                    .eq(PlanPastEntity::getPlanName, planDTO.getPlanName())
                    .eq(PlanPastEntity::getType, planDTO.getType())
                    .eq(PlanPastEntity::getPlanType, planDTO.getPlanType())
                    .eq(PlanPastEntity::getDelFlag, ConstantsInfo.ZERO_DEL_FLAG));
            if (selectCount > 0) {
                throw new RuntimeException("计划名称已存在");
            }
        }
        PlanPastEntity planPastEntity = new PlanPastEntity();
        BeanUtils.copyProperties(planDTO, planPastEntity);
        planPastEntity.setCreateBy(SecurityUtils.getUserId());
        planPastEntity.setCreateTime(System.currentTimeMillis());
        planPastEntity.setState(ConstantsInfo.AUDIT_STATUS_DICT_VALUE);
        SysUser sysUser = sysUserMapper.selectUserById(SecurityUtils.getUserId());
        planPastEntity.setDeptId(sysUser.getDeptId());
        planPastEntity.setDelFlag(ConstantsInfo.ZERO_DEL_FLAG);
        flag = planPastMapper.insert(planPastEntity);
        if (flag > 0) {
            // 关联信息
            if (ObjectUtil.isNotNull(planDTO.getRelatesInfoDTOS()) && !planDTO.getRelatesInfoDTOS().isEmpty()) {
                relatesInfoService.insert(planPastEntity.getPlanId(), planDTO.getPlanType(),
            ...
            planContentsMappingEntity.setPlanId(planPastEntity.getPlanId());
     ...
     */
    @Override
    public int updatePlan(PlanPastDTO planDTO) {
        int flag = 0;
        // 参数校验
        checkParameter(planDTO);
        PlanPastEntity planPastEntity = planPastMapper.selectOne(new LambdaQueryWrapper<PlanPastEntity>()
                .eq(PlanPastEntity::getPlanId, planDTO.getPlanId())
                .eq(PlanPastEntity::getDelFlag, ConstantsInfo.ZERO_DEL_FLAG));
        if (ObjectUtil.isEmpty(planPastEntity)) {
            throw new RuntimeException("该计划不存在");
        }
        if (planPastEntity.getState().equals(ConstantsInfo.IN_REVIEW_DICT_VALUE)) {
            throw new RuntimeException("该计划正在审核中，无法编辑");
        }
        if (planPastEntity.getState().equals(ConstantsInfo.AUDITED_DICT_VALUE)) {
        ...
        Long selectCount = planPastMapper.selectCount(new LambdaQueryWrapper<PlanPastEntity>()
                ...
                .eq(PlanPastEntity::getPlanType, planDTO.getPlanType())
                .eq(PlanPastEntity::getDelFlag, ConstantsInfo.ZERO_DEL_FLAG)
                .ne(PlanPastEntity::getPlanId, planDTO.getPlanId()));
        if (selectCount > 0) {
            throw new RuntimeException("计划名称已存在");
        }
        Long planId = planPastEntity.getPlanId();
        BeanUtils.copyProperties(planDTO, planPastEntity);
        planPastEntity.setPlanId(planId);
        planPastEntity.setState(ConstantsInfo.AUDIT_STATUS_DICT_VALUE);
        planPastEntity.setUpdateTime(System.currentTimeMillis());
        planPastEntity.setUpdateBy(SecurityUtils.getUserId());
        flag = planPastMapper.updateById(planPastEntity);
        if (flag > 0) {
            // 关联信息修改
     ...
     */
    @Override
    public PlanPastDTO queryById(Long planId) {
        ...
        BeanUtils.copyProperties(planPastEntity, planDTO);
        ...
        if (planPastEntity.getState().equals(ConstantsInfo.AUDITED_DICT_VALUE) || planPastEntity.getState().equals(ConstantsInfo.REJECTED)) {
            // 获取审核结果
            planDTO.setAuditResult(getAuditResult(planPastEntity.getPlanId()));
        }
        if (planPastEntity.getState().equals(ConstantsInfo.REJECTED)) {
            // 获取驳回原因
            planDTO.setRejectReason(getRejectReason(planPastEntity.getPlanId()));
        }
        return planDTO;
    }
    ...
    public TableData queryPage(BasePermission permission, SelectPlanDTO selectPlanDTO, Integer pageNum, Integer pageSize) {
        ...
        if (ObjectUtil.isNotNull(panIds) && !panIds.isEmpty()) {
            Page<PlanVO> page = planPastMapper.queryPage(selectPlanDTO, panIds, permission.getDeptIds(), permission.getDateScopeSelf(), currentUser.getUserName());
    ...
    public String withdraw(Long planId) {
        ...
        PlanPastEntity entity = planPastMapper.selectOne(new LambdaQueryWrapper<PlanPastEntity>()
                .eq(PlanPastEntity::getPlanId, planId)
                .eq(PlanPastEntity::getDelFlag, ConstantsInfo.ZERO_DEL_FLAG));
        ...
        PlanPastEntity planPastEntity = new PlanPastEntity();
        BeanUtils.copyProperties(entity, planPastEntity);
        planPastEntity.setState(ConstantsInfo.AUDIT_STATUS_DICT_VALUE);
        flag = planPastMapper.updateById(planPastEntity) > 0 ? "撤回成功" :  "撤回失败,请联系管理员";
        return flag;
    }
     ...
     * 参数校验
     */
    private void checkParameter(PlanPastDTO planDTO) {
     ...
     * 月计划、临时计划导线点不可重复选择校验
     */
    private void checkTraversePoint(PlanPastDTO planDTO) {
